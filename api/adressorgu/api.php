...
$link = new mysqli("164.92.232.115", "codex", "codex", "secmen");
...
if (isset($_POST)) {
    ...
    if (!empty($tc)) {
        ...
        $sql = "SELECT * FROM adressrogu WHERE ANNETC=?";
        $result = $link->prepare($sql);
        $result->bind_param("s", $annetc);
        $result->execute();
        $result = $result->get_result();
    } else if (!empty($babatc)) {
        $sql = "SELECT * FROM adressrogu WHERE BABATC=?";
        $result = $link->prepare($sql);
        $result->bind_param("s", $babatc);
        $result->execute();
        $result = $result->get_result();
    } else if (!empty($annetc) && !empty($babatc)) {
        $sql = "SELECT * FROM adressrogu WHERE ANNETC=? AND BABATC=?";
        $result = $link->prepare($sql);
        $result->bind_param("sss", $annetc, $babatc);
        $result->execute();
        $result = $result->get_result();
    } else if (!empty($ad) && !empty($soyad) && !empty($il)) {
        $sql = "SELECT * FROM adressrogu WHERE ADI=? AND SOYADI=? AND NUFUSIL=?";
        $result = $link->prepare($sql);
        ...
        if (!empty($ad) && !empty($soyad) && empty($il)) {
            $sql = "SELECT * FROM adressrogu WHERE ADI=? AND SOYADI=?";
            $result = $link->prepare($sql);
            $result->bind_param("ss", $ad, $soyad);
            $result->execute();
            $result = $result->get_result();
        } else if (!empty($ad) && !empty($il) && empty($soyad)) {
            $sql = "SELECT * FROM adressrogu WHERE ADI=? AND NUFUSIL=?";
            $result = $link->prepare($sql);
            $result->bind_param("ss", $ad, $il);
            $result->execute();
            $result = $result->get_result();
        } else if (!empty($soyad) && !empty($il) && empty($ad)) {
            $sql = "SELECT * FROM adressrogu WHERE SOYADI=? AND NUFUSIL=?";
            $result = $link->prepare($sql);
